...
class drawLineFromTo{
  ...
  startdrawLineFromTo(_this,thisSet){
    ...
    this.connectPath = R.path(`M${this.ox} ${this.oy}L${this.ox} ${this.oy}`).attr({stroke:"blue"});
    ...
    GlobalStorage.currentLine=this.connectPath;//Send just created Line into GlobalStorage object currentLine
...
this.pathCoords=bezieLine(this.ox,this.oy,this.destX,this.destY);

  GlobalStorage.currentLine.attr({d:`M${this.ox} ${this.oy}C${this.pathCoords.cp1x} ${this.pathCoords.cp1y} ${this.pathCoords.cp2x} ${this.pathCoords.cp2y} ${this.destX} ${this.destY}`});
  ...
  endDrawLine(_this,thisSet){
    ...
    if (GlobalStorage.overMouseSet!==null&&GlobalStorage.currentLine!==null&&!GlobalStorage.overMouseSet.fullCommonContrlName&&GlobalStorage.controlProp.type==GlobalStorage.currentLine.node.shortControlName){// in this case the current Line has connection from CommonControl to EffectBlock
      ...
      this.pathCoords=bezieLine(MX,MY,LX,LY);
      //console.log(pathCoords);
      GlobalStorage.currentLine.attr({d:`M${MX} ${MY}C${this.pathCoords.cp1x} ${this.pathCoords.cp1y} ${this.pathCoords.cp2x} ${this.pathCoords.cp2y} ${LX} ${LY}`});// apply new coords
